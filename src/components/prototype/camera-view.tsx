 ...
 *
 * This component runs the pinned YOLOS-tiny detector on real video frames.
 ...
 *
 * The model is code-split so the browser adapter loads only when this
...
export function CameraView() {
  ...
  // ===== Agent loop (shared) =====
  const runAgentLoop = (canvas: HTMLCanvasElement | null, dets: Detection[], frameDiffScore: number) => {
    ...
    // based on whether the specialized model or base detector found something.
    // Check both the specializedClassName (for HF model detections) AND
    // detectionClasses (for detector classes like person/car/backpack).
    ...
    void agentActions.executeSequentially(decision.actions, {
      cameraId: activeCamera.id,
      cameraLabel: activeCamera.label,
      stats: state.stats,
      detections: dets,
  ...
  }, [activeCameraId, clearSamples, setAppearanceTracks])

  // ===== Real ML detection loop =====
  // ARCHITECTURE: pinned YOLOS-tiny runs in the main loop.
  ...
  useEffect(() => {
    ...
    const loop = async () => {
      ...
      try {
        ...
        // ─── Step 1: pinned YOLOS-tiny detection ───
        let dets: Detection[] = []
        let latency = 0
        if (runYolos) {
        ...
        let frameDiffScore = 0

        if (ctx) {
          frameDiffScore = computeFrameDifferenceScore(
            ctx,
            canvas.width,
            canvas.height,
            `${activeCamera.id}:${useCase?.id ?? 'none'}`,
          )
          ...
          if (useCase && runHFModels && hasSpecializedModel(useCase.id) && !hfInFlight) {
            ...
            runSpecializedDetectionEnsemble(canvas, useCase.id, userModels)
          ...
          if (fpsTick.n % 5 === 0) {
            const localTracks = appearanceTracker.getIdentities().map((id) => ({
              trackId: id.trackId,
              type: id.type,
        ...
        runAgentLoop(canvas, dets, frameDiffScore)
  ...
  const handleModelStatus = useCallback((status: 'loading' | 'ready' | 'error', error: string | null = null) => {
    setModelStatus(status, error)
  }, [setModelStatus])
  const handleModelReady = useCallback((handle: RealMlHandle | null) => {
