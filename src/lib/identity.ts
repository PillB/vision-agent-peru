/**
 * Within-video appearance tracking helpers.
 *
 ...
 *   - Local track continuity: centroid-based gallery with cosine similarity matching
 *   - Person/vehicle appearance cues: bbox aspect ratio, dominant color, size
...
/** A local appearance track for a person or vehicle. Never an identity. */
...
/**
 * Local appearance-track manager. The caller resets it on every source
 * change. Its color histogram and bbox geometry are weak continuity cues only;
 * they never establish identity or authorize an action.
 */
export class AppearanceTracker {
  ...
  /**
   * Match or create a local appearance track for a tracked object.
   * Returns a session-local track ID.
   */
  matchOrCreate(
    localTrackId: number,
    type: 'person' | 'vehicle',
    ...
    if (bestMatchId && bestScore >= this.matchThreshold) {
      ...
      // Create new identity
      gid = `local-track-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`
